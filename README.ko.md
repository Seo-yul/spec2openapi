...
| XSD 파셋 | enumeration, pattern, length/minLength/maxLength, min/maxInclusive, min/maxExclusive, fractionDigits(→`x-fractionDigits`). xsd:import/include된 외부 스키마 포함. 결정적 `example` 값 부여(첫 enumeration 값, 날짜/시간 표기 예시 상수 — 합성 없음) |
