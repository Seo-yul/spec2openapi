...
convert 옵션: `--openapi-version 3.0|3.1`(기본 `3.0`, 출력 스펙에는 `openapi: 3.0.3`으로 기록), `--service`/`--port-name`, `--prefer-soap12`, `--base-path`, `--title`, `--strict`(미지원 오퍼레이션 스킵 대신 실패), `--forbid-external`(원격 wsdl:/xsd: import fetch 거부 — 신뢰할 수 없는 WSDL 변환 시 권장, 로컬 상대경로 import는 계속 동작), `--huge-tree`(초대형 WSDL용 libxml2 제한 해제).
...
| 타입 | 중첩 complexType($ref), 배열(maxOccurs→array/minItems/maxItems), attribute, nillable, anyType, 상속(complexContent extension은 평탄화), simpleContent(값+attribute), choice(멤버 optional화 + `x-soap-choice`), default 값 |
...
정보가 부족한 경우의 처리 정책은 3단계다.

1. 합의된 기본값을 결정적으로 적용하고 루트 `x-s2o.assumptions`에 전부 기록한다: `consumes`/`produces` 생략 → `application/json` 가정, `operationId` 없음 → `{method}_{path}` 규칙으로 생성(FastMCP tool 이름에 필수), `host` 없음 → 상대 서버 `/` + 런타임 엔드포인트 오버라이드에 위임, `schemes` 없음 → https 가정.
2. 등가물이 없는 구조(collectionFormat `tsv` 등)는 버리지 않고 `x-` 확장으로 보존하며 `x-s2o.lossy`에 기록한다.
3. 최종 게이트는 `spec2openapi validate`다. 가정이 들어간 스펙도 FastMCP 라운드트립으로 tool 생성까지 실제 확인한다. tool 생성에 필요한 것은 paths/스키마이고 서버·인증은 런타임 관심사라, 가정이 tool 생성을 막는 경우는 없다.

`upgrade` 실행 시 assumptions/lossy 목록이 stderr로 출력되고, `validate`/`serve`는 Swagger 2.0 입력을 자동 감지해 메모리에서 업그레이드한다.

## FastMCP 호환 보장

이 프로젝트가 보장하는 계약: 생성된 스펙은 `FastMCP.from_openapi()`를 통과해 오퍼레이션 수만큼의 MCP tool이 만들어진다.
...
`Dockerfile`(고정 이미지)과 `k8s/example.yaml`(ConfigMap으로 스펙 교체 + Secret 자격증명)이 쿠버네티스 운영 예시다. 자체 런타임을 만들 경우 `src/spec2openapi/bridge.py`를 참조 구현으로 쓰면 된다.
...
테스트 스위트 구성: 변환 단위 테스트, Swagger 업그레이드 테스트, envelope 직렬화/역직렬화, 목 SOAP 서버 상대 e2e(MCP tool 호출→SOAP 왕복, rpc/simpleContent/choice/재귀 타입/unqualified form 포함), 전체 픽스처 x OpenAPI 3.0/3.1 FastMCP 라운드트립, 그리고 실전 난제 패턴 스트레스 테스트(재귀/순환 $ref, 4단 중첩, 대형 enum, 네임스페이스 간 타입명 충돌, 서비스 간 동명 오퍼레이션, 특수문자 경로, 깊은 allOf 체인). `examples/`에 생성된 스펙 샘플이 있다.
