"""Optional MCP runtime glue (requires the [mcp] extra: fastmcp + httpx2).
...
def from_openapi_spec(
    ...
    **fastmcp_kwargs: Any,
    ...
    plain REST specs get a regular httpx2 client (one runtime serves both).
    """
    try:
        import httpx2
    ...
    if spec_has_soap(spec):
        ...
        client = httpx2.AsyncClient(transport=transport, base_url=BRIDGE_BASE_URL)
    else:
        options = options or BridgeOptions.from_env()
        base = options.endpoint or (spec.get("servers") or [{}])[0].get("url", "")
        client = httpx2.AsyncClient(base_url=base, **_client_kwargs(options))
