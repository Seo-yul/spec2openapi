"""SOAP bridge: an httpx2 transport that turns REST/JSON calls generated by
...
FastMCP tool call -> POST {base}/operations/{op} (JSON)
    -> [this transport] JSON -> SOAP envelope (metadata from x-soap + xml
       ...
       JSON (shaped by the response schema) -> httpx2.Response(200, json)
...
def _basic_auth(options: BridgeOptions) -> tuple[str, str] | None:
    """The (username, password) tuple for httpx2's `auth=` kwarg, or None
...
def _client_kwargs(options: BridgeOptions) -> dict[str, Any]:
    """httpx2.AsyncClient kwargs derived from BridgeOptions: auth, timeout,
...
def _coerce(text: str | None, schema: dict[str, Any]) -> Any:
    ...
    try:
        ...
        if t == "number":
            v = float(s)
            # xsd NaN/INF/-INF parse fine but aren't valid JSON; httpx2's
            # encoder rejects them with ValueError, so keep the original
            # text instead of a non-finite float
...
class SoapBridgeTransport(httpx2.AsyncBaseTransport):
    """httpx2 transport that answers FastMCP's REST calls by calling SOAP."""
    ...
    def __init__(self, spec: dict[str, Any], options: BridgeOptions | None = None):
        ...
        self._client = httpx2.AsyncClient(**_client_kwargs(self.options))

    async def aclose(self) -> None:
        await self._client.aclose()

    def _json_response(self, request: httpx2.Request, status: int,
                       payload: dict[str, Any]) -> httpx2.Response:
